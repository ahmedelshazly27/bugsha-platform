...
export default function D() {
  ...
  return <>
    ...
    {[KPI.slice(0, 4), KPI.slice(4)].map((row, i) => <View key={i} style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>{row.map(([k, label, fmt, tone]) => <PartnerStat key={k} label={label} value={fmt(d[k], m)} tone={k === 'jobs_alerting' && Number(d[k]) > 0 ? 'urgent' : k === 'open_disputes' && Number(d[k]) > 0 ? 'urgent' : tone ?? 'neutral'} style={{ minWidth: 160, flex: 1 }} />)}</View>)}
    ...
    <Panel style={{ padding: 14, gap: 8 }}>
      {sdRows.length === 0 ? <Caption>No listings or orders in the window.</Caption> : sdRows.map((r: any) => <View key={r.day} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        ...
        <Caption style={{ width: 300 }} numberOfLines={1}>{`${r.bags ?? 0} bags · ${r.orders ?? 0} orders · ${r.listings ?? 0} listings${r.sell_through != null ? ' · ' + Math.round(Number(r.sell_through) * 100) + '% sold' : ''}`}</Caption></View>)}
