...
// New kitchens join with a partner code from ops (/join) — `next` carries them back to the application after sign-in.
import { useState } from 'react'; import { router, useLocalSearchParams } from 'expo-router'; import { Pressable, View } from 'react-native';
...
export default function SignIn() {
  const { L, lang } = useL(); const { next } = useLocalSearchParams<{ next?: string }>(); const [email, setEmail] = useState(''); const [otp, setOtp] = useState(''); const [sent, setSent] = useState(false); const [err, setErr] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  return <View style={{ flex: 1 }}><Screen scroll={false} style={{ justifyContent: 'center', gap: 16, padding: 24 }}>
    ...
    <Foot>{!sent ? <Button size="lg" fullWidth loading={busy} disabled={!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)} onPress={async () => { setBusy(true); const { error } = await db.auth.signInWithOtp({ email: email.trim().toLowerCase() }); setBusy(false); if (error) return setErr(L("Couldn't send the code. Try again in a minute.", 'تعذّر إرسال الرمز. حاول بعد دقيقة.')); setErr(null); setSent(true); }}>{L('Send code', 'إرسال الرمز')}</Button>
      : <Button size="lg" fullWidth loading={busy} disabled={otp.length < 6} onPress={async () => { setBusy(true); const { error } = await db.auth.verifyOtp({ email: email.trim().toLowerCase(), token: otp, type: 'email' }); setBusy(false); if (error) return setErr(L("That code didn't match", 'الرمز غير مطابق')); router.replace((next ? String(next) : '/(tabs)') as any); }}>{L('Sign in', 'تسجيل الدخول')}</Button>}
      <Pressable accessibilityRole="link" onPress={() => router.push('/join')}><Caption align="center">{L('Not a partner yet? Join with a partner code', 'لست شريكاً بعد؟ انضم برمز شريك')}</Caption></Pressable></Foot></View>;
}
