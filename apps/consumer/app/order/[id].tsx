...
export default function Order() {
  ...
  return <View style={{ flex: 1 }}><AppBar title={redeemed ? L('Collected', 'تم الاستلام') : reserved ? L('Reserved', 'محجوزة') : L('Order', 'الطلب')} onBack={() => router.back()} action={<IconButton icon="info" label={L('Help', 'مساعدة')} />} />
    <Screen top={false} pad={16} gap={14}>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}><Button variant="secondary" style={{ flex: 1 }} iconStart="receipt" onPress={() => router.push({ pathname: '/order/receipt', params: { id } })}>{L('Receipt', 'الإيصال')}</Button><Button variant="secondary" style={{ flex: 1 }} iconStart="triangle-alert" onPress={() => router.push({ pathname: '/order/report', params: { id } })}>{L('Report a problem', 'إبلاغ عن مشكلة')}</Button></View>
