...
export default function Intro() {
  const { market, set } = useSession(); const { L, ar } = useL(); const [i, setI] = useState(0);
  const panels = [
  ...
  return <View style={{ flex: 1 }}><View style={{ alignItems: 'flex-end', paddingTop: 54, paddingHorizontal: 14 }}><Button variant="ghost" size="sm" onPress={() => { set({ introSeen: true }); router.replace('/'); }}>{L('Skip', 'تخطي')}</Button></View>
    ...
    <Foot><View style={{ flexDirection: 'row', gap: 6, justifyContent: 'center', paddingBottom: 4 }}>{[0, 1, 2].map((n) => <View key={n} style={{ width: n === i ? 20 : 6, height: 6, borderRadius: 99, backgroundColor: n === i ? color.brand : color.border }} />)}</View>
      <Button size="lg" fullWidth iconEnd="arrow-right" onPress={() => { if (i === 2) { set({ introSeen: true }); router.replace('/'); } else setI(i + 1); }}>{i === 2 ? L("See tonight's bags", 'شاهد بقش الليلة') : L('Next', 'التالي')}</Button></Foot></View>;
}
