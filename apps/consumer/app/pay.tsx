...
import { useOrder, useReserveCash, newIdempotencyKey } from '@bugsha/api';
import { AppBar, Banner, Button, Caption, Card, Eyebrow, Foot, Line, Num, PaymentMethodRow, Rule, Screen, T, color } from '@bugsha/ui';
...
export default function Pay() {
  const { order } = useLocalSearchParams<{ order: string }>(); const { market } = useSession(); const { L } = useL();
  ...
  return <View style={{ flex: 1 }}><AppBar title={L('Payment', 'الدفع')} onBack={() => router.back()} />
    <Screen top={false} pad={16} gap={12}>
      {o ? <Card><View style={{ gap: 6 }}><Line k={`${o.title_snapshot} × ${o.quantity}`} v={<Num role="label">{money(o.subtotal_minor, market)}</Num>} />{o.discount_minor ? <Line k={L('Discount', 'خصم')} v={<Num role="label">{`−${money(o.discount_minor, market)}`}</Num>} /> : null}<Rule /><Line k={L('Total', 'الإجمالي')} v={<Num weight={700}>{money(o.total_minor, market)}</Num>} strong /></View></Card> : null}
