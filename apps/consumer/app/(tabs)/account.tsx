...
import { useProfile, useWallet, rpc } from '@bugsha/api';
...
export default function Account() {
  const s = useSession(); const { L, ar } = useL(); const wallet = useWallet(db) as any; const profile = useProfile(db) as any; const p = profile.data ?? {};
  const name = [p.first_name, p.last_name].filter(Boolean).join(' '); const initial = (name || (ar ? 'ب' : 'B')).slice(0, 1).toUpperCase();
  ...
  return <View style={{ flex: 1 }}><AppBar title={L('Account', 'حسابي')} back={false} />
    <Screen top={false} pad={0} gap={0}>
      <View style={{ padding: 16, flexDirection: 'row', gap: 12, alignItems: 'center', backgroundColor: color.raised }}><View style={{ width: 46, height: 46, borderRadius: 99, backgroundColor: color.brandTint, alignItems: 'center', justifyContent: 'center' }}><T weight={700} color={color.brand} style={{ fontSize: 18, lineHeight: 24 }}>{initial}</T></View><View style={{ flex: 1, minWidth: 0 }}><T weight={600} numberOfLines={1}>{name || L('Your account', 'حسابك')}</T><Caption numberOfLines={1}>{p.email ?? (s.market === 'KW' ? L('Kuwait', 'الكويت') : L('Egypt', 'مصر'))}</Caption></View></View>
