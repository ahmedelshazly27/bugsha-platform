...
import { useState } from 'react'; import { router, useLocalSearchParams } from 'expo-router'; import { Share, View } from 'react-native';
import { useHold, useListing, useProfile, useSavedStores, useToggleSaved, RpcError } from '@bugsha/api'; import { errorMessage } from '@bugsha/i18n';
...
export default function Listing() {
  ...
  const { userId } = useSession(); const saved = useSavedStores(db); const toggle = useToggleSaved(db); const profile = useProfile(db, !!userId) as any; const restricted = !!profile.data?.restricted_until;
  ...
  const total = major(l.price_minor, market) * qty; const soldOut = l.quantity_remaining <= 0; const isSaved = ((saved.data ?? []) as any[]).some((x) => x.store_id === l.store_id);
  const share = () => Share.share({ message: L(`${l.store_name} has a ${bagTitle(l.category, false).toLowerCase()} on Bugsha tonight for ${cur.code} ${major(l.price_minor, market).toFixed(cur.dp)} — bugsha://listing/${id}`, `${l.store_name} يعرض ${bagTitle(l.category, true)} على بقشة الليلة بـ ${cur.code} ${major(l.price_minor, market).toFixed(cur.dp)} — bugsha://listing/${id}`) }).catch(() => {});
  ...
  return <View style={{ flex: 1, backgroundColor: color.canvas }}>
    <CoverPlate category={l.category} height={190} radius={0} label={undefined}>
      ...
      <View style={{ position: 'absolute', top: 54, end: 8, flexDirection: 'row', gap: 6 }}><IconButton icon="heart" variant="solid" label={isSaved ? L('Saved', 'محفوظ') : L('Save store', 'حفظ المتجر')} color={isSaved ? '#C4B5FD' : '#fff'} onPress={() => (userId ? toggle.mutateAsync(l.store_id).then(() => saved.refetch()) : router.push('/auth/email'))} /><IconButton icon="share-2" variant="solid" label={L('Share', 'مشاركة')} onPress={share} /></View>
    ...
    <Foot><Button size="lg" fullWidth disabled={soldOut || restricted} loading={hold.isPending} onPress={reserve}>{L(`Reserve for ${cur.code} ${total.toFixed(cur.dp)}`, `احجز بـ ${cur.code} ${total.toFixed(cur.dp)}`)}</Button>
