...
export const useCompleteProfile = (db: Bugsha) => useMutation({ mutationFn: (a: { firstName: string; market: string; cityId?: string }) => rpc(db, 'complete_profile', { p_first_name: a.firstName, p_market: a.market, p_city_id: a.cityId ?? null }) });
