...
/** Layout direction the apps are running in. The root layout sets it from the chosen locale (setRTL) so mirroring
 * follows the language immediately; without a call it falls back to the native I18nManager flag (needs a restart). */
let forcedRTL: boolean | null = null;
export const setRTL = (rtl: boolean) => { forcedRTL = rtl; };
export const isRTL = () => forcedRTL ?? I18nManager.isRTL;
