...
## D39 — Partner sign-up is code-gated; the database sends the code (2026-09-14)
A partner account opens only with a single-use `BG-XXXX-XXXX` code issued by ops (`ops_manager` or `admin`) against a request the kitchen made on bugsha.app/partners or in the partner app. `app.submit_application` requires the code (`BG122` without one, `BG123` for the wrong market) and the code-less signature is gone, so no client can bypass the gate. Codes expire after 14 days by default, are tied to the request's email, and can be revoked while live; a redeemed code is never undone from the code side (`BG126`) — suspend the partner. Issuing inserts into `partner_invite_code`, and an `AFTER INSERT` trigger posts to the `partner-code-email` function through pg_net, signed with a Vault secret, so the email goes out without a person in the loop and delivery is recorded on the row. The queue lives in the ops console (`/requests`) and at bugsha.app/ops. Gate error codes sit at `BG122`–`BG128`; `BG130`/`BG131` stay four-eyes / re-auth. Migrations `20260914090455`, `20260914094502`, `20260914095840`, `20260914110851`, `20260914111830`.

## D40 — Pickup QR carries a deep link; late hand-over is an explicit, online-only path (2026-09-14)
The consumer's reserved order shows a QR encoding `bugsha://redeem/<CODE>` beside the 5-character code. The partner app scans it with `expo-camera` and redeems with mechanism `qr_scanned`; a typed code stays `code_shown`. Both run through the offline mirror. Past the market's late-redeem grace (30 min) the screen no longer says "window closed": an online staff member can record a late hand-over through `app.redeem_order_late`, which is logged for the customer and ops; offline it explains that a connection is needed. Undo is 60 s to match `market_config.undo_redeem_seconds`.

## D41 — Client surface for the screen pass lives in one migration (2026-09-14)
`20260914133248_client_surface.sql` adds what the screens needed and the RPCs did not return: `my_profile()` restriction/deletion fields, `cancel_deletion()`, `my_partner()` history and suspension, `my_quality_flags()`, `acknowledge_flag()`, `my_disputes()`, `respond_dispute()`. Ops-side gaps (refund preview, task assignment, acquisition lists, a quality-hold list) stay unexposed and are listed in `09-screens.md` rather than stubbed in the apps.
